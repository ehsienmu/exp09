...
#     """
#     def __init__(self, dataset='ImageNet' ,rounding=torch.round, factor=0 ,cut_off = 0):
...
class block_dct(nn.Module):
    ...
    Ouput:
        DCT coefficients : batch x 192 x height/8 x width/8)
    """
    def __init__(self, dataset=None ,rounding=torch.round, factor=0 ,cut_off = 0, img_height=256):
        super(block_dct, self).__init__()
    ...
    def forward(self, image):
        ...
        if(dim == 3):
            dct_coff = torch.stack((components['r'],components['g'],components['b']))
            dct_coff = torch.permute(dct_coff , (1,2,3,4,0)) 
            # print("dct_coff.size():", dct_coff.size())
